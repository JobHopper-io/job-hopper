...
- **Key relationships**:
  - One organization can have many `users` (via `users.organization_id`).
...
### users
- **Meaning**: End users of the app who belong to an organization and use job‑hopping features.
- **Key relationships**:
  - Many users can belong to a single `organization` (`users.organization_id` → `organizations.id`).
- **Non‑obvious rules**:
...
- **Key relationships**:
  - These tables currently have no explicit foreign‑key links to `users` or `organizations` in the generated types; linkages are done via shared fields (e.g. company name, job metadata) or app‑level logic.
- **Non‑obvious rules**:
