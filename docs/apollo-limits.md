...
| `career_partner_discovery` | Keyword+location org discovery for career-partner leads (`scripts/career-partner-connector.mjs`) — one search per category/metro combo, not per-lead | 1 credit: `mixed_companies/search` per category/metro search (no `people/match` step — this connector only discovers organizations, not individual contacts) |
