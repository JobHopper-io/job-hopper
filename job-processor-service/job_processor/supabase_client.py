...
class SupabaseRest:
    ...
    async def count_rows(self, table: str, *, params: list[tuple[str, str]] | None = None) -> int:
        ...
        doing it) and post-apply verification (e.g. counting orphans)."""
        query = [("select", "id"), ("limit", "0")] + list(params or [])
        r = await self._client.request(
            "GET",
            f"{self._rest}/{table}",
